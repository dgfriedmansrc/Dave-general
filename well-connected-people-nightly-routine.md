...
DATA-SAFETY RULE (non-negotiable): "Connected to" is APPEND-ONLY. Never remove, replace, clear, or overwrite an existing value in any person's "connected_to" field. Every write MUST use patch_multiselect_values=true and MUST contain only additions. If you cannot confirm a write will preserve all existing values, do not perform it. A link that is already present is left exactly as-is, even if you did not re-derive it this run. This field is a permanent, growing record of who each person is connected to — losing an entry is a critical failure.
...
5. If the union is non-empty: update-record object "people", record_id=M, patch_multiselect_values=true, values={"connected_to":[{"target_object":"people","target_record_id":"<id>"}, ...]}. patch_multiselect_values=true is REQUIRED — it appends and preserves every existing entry; never send this write with patch off or with a full replacement value. Pass only the newly-found additions. Always write to M (the record being processed), even when a match came from a duplicate. If the union is empty, write nothing for M.
